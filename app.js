...
function renderWorkout(){
  ...
  currentWorkout=name;
  const arr=PLAN[name];
  let html=`<div class="card"><div class="row between"><div><b>${esc(name)}</b><div class="muted">${phase(week).name} • ${phase(week).rir}</div></div><button class="good" onclick="finishWorkout()">Salvar treino</button></div><div class="progress" style="margin-top:10px"><b>Revisão dos exercícios:</b> reavaliar a seleção a cada 8–12 semanas, sem troca automática. Mantenha enquanto houver progressão, boa técnica, conforto articular e estímulo no músculo-alvo. Troque antes apenas por dor/desconforto persistente, estagnação real após ajustes, execução que não encaixa ou equipamento indisponível.</div></div>`;
  arr.forEach((ex,idx)=>{
