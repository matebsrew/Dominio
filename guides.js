...
function inject(){
 ...
 document.querySelectorAll('.exercise').forEach((card,i)=>{
  ...
  const url=IMG[ex.name];const fileName=url?decodeURIComponent(url.split("/").pop()):"";const credit=url?`https://commons.wikimedia.org/wiki/File:${encodeURIComponent(fileName)}`:"";box.innerHTML=`${url?`<img src="${url}" alt="Anatomia: ${d[0]}" loading="lazy" referrerpolicy="no-referrer" onerror="this.nextElementSibling?.remove();this.remove()"><a class="anatomyCredit" href="${credit}" target="_blank" rel="noopener noreferrer">Fonte anatômica e licença</a>`:''}<h4>🎯 ${d[0]}</h4><ul><li>${d[1]}</li><li>${d[2]}</li></ul><h4>Evite</h4><ul class="bad"><li>${d[3]}</li></ul>`;
